...
(function() {
  ...
  var InteractiveMethods = /** @scope fabric.Canvas.prototype */ {
	...
	_scaleObjectX: function (target, currentLength, lastLength) {
        var t = this._currentTransform;
        var newScalingFactor = t.scaleX * currentLength / lastLength;
        target.set('scaleX', newScalingFactor);

        if (target.scaleX == newScalingFactor) {
          var scaleOrigin = (target.__corner == 'tr' || target.__corner == 'mr' || target.__corner == 'br') ? fabric.ScaleOrigin.TopLeft : fabric.ScaleOrigin.TopRight;
          target._updatePositionForScaleX(t.scaleX, newScalingFactor, scaleOrigin, t);
        } 
		
    },

    _scaleObjectY: function (target, currentLength, lastLength) {
        var t = this._currentTransform;
        var newScalingFactor = t.scaleY * currentLength / lastLength;
        target.set('scaleY', newScalingFactor);

        if (target.scaleY == newScalingFactor) {
          var scaleOrigin = (target.__corner == 'bl' || target.__corner == 'bm' || target.__corner == 'br') ? fabric.ScaleOrigin.TopLeft : fabric.ScaleOrigin.BottomLeft;
          target._updatePositionForScaleY(t.scaleY, newScalingFactor, scaleOrigin, t);
        }
    },
    ...
    _scaleObject: function (x, y, by) {
      ...
      if (!by) {
        target.lockScalingX || this._scaleObjectX(target, curLen, lastLen);
        target.lockScalingY || this._scaleObjectY(target, curLen, lastLen);
      }
  ...
  fabric.Object = fabric.util.createClass(/** @scope fabric.Object.prototype */ {
    ...
    toObject: function(propertiesToInclude) {
      ...
      var object = {
        ...
        useSideAsScalingOrigin: this.useSideAsScalingOrigin,
	 ...
	 * @param scaleOrigin {fabric.ScaleOrigin} optional origin for the scaling
     * @return {fabric.Object} thisArg
     * @chainable
     */
    scale: function(value, scaleOrigin) {
	  
	  if (scaleOrigin && this.useSideAsScalingOrigin) {
	    this._updatePositionForScaleX(this.scaleX, value, scaleOrigin, this);
		this._updatePositionForScaleY(this.scaleY, value, scaleOrigin, this);
      }
	...
	_updatePositionForScaleX: function(oldScalingFactor, newScalingFactor, scalingOrigin, oldPosition) {
	  if (this.useSideAsScalingOrigin) {
	    if (fabric.ScaleOrigin.isLeft(scalingOrigin) || fabric.ScaleOrigin.isRight(scalingOrigin)) {
          var diff = (newScalingFactor - oldScalingFactor) * this.width;
          var offset = (fabric.ScaleOrigin.isRight(scalingOrigin)) ? diff / -2 : diff / 2;
          this.set('left', oldPosition.left + offset);
        }
      } 
    },
    
    _updatePositionForScaleY: function(oldScalingFactor, newScalingFactor, scalingOrigin, oldPosition) {
	  if (this.useSideAsScalingOrigin) {
        if (fabric.ScaleOrigin.isTop(scalingOrigin) || fabric.ScaleOrigin.isBottom(scalingOrigin)) {
	      var diff = (newScalingFactor - oldScalingFactor) * this.height;
          var offset = (fabric.ScaleOrigin.isBottom(scalingOrigin)) ? diff / -2 : diff / 2;
          this.set('top', oldPosition.top + offset);
        }
      } 
    },
     ...
     * @chainable
     */
    scaleToWidth: function(value, scaleOrigin) {
      // adjust to bounding rect factor so that rotated shapes would fit as well
      var boundingRectFactor = this.getBoundingRectWidth() / this.getWidth();
      return this.scale(value / this.width / boundingRectFactor, scaleOrigin);
    },
     ...
     * @chainable
     */
    scaleToHeight: function(value, scaleOrigin) {
      // adjust to bounding rect factor so that rotated shapes would fit as well
      var boundingRectFactor = this.getBoundingRectHeight() / this.getHeight();
      return this.scale(value / this.height / boundingRectFactor, scaleOrigin);
    },
