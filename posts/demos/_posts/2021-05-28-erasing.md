...
The `deep` option introduces fine grained control over a group's `erasable` property.
  ...
  - When set to `true` the eraser will erase the entire group. Once the group changes (an object is added/removed) the eraser is propagated to its children for proper functionality.
